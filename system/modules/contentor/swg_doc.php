...
----------------------------------------------------------------------------
$Id: swg_doc.php,v 1.10 2009/03/28 07:52:55 s4u Exp $
...
case "edit-save":
{
	...
	else { $g_source_url = "m=contentor&a=view&dsd=[oid]"; }
	...
	if ($g_doc_array)
	{
		if (isset ($direct_settings["contentor_".$g_doc_array['ddbcontentor_docs_doctype']]))
		{
			if ($g_doc_array['ddbdatalinker_id_main'])
			{
				...
				if (($g_cat_array)&&(($g_doc_object->is_writable ())||(($g_doc_object->is_writable_group ())&&($direct_classes['kernel']->v_group_user_check_group ($g_cat_array['ddbcontentor_cats_owner_group']))))&&($g_cat_object->is_writable ())) { $g_rights_check = true; }
			}
			else
			{
	...
	elseif ($g_rights_check)
	{
		...
		if ((!$g_datasub_check)&&($g_doc_array['ddbcontentor_docs_doctype'] == "wiki"))
		{
			...
			$direct_classes['formtags']->define_connector ("m=contentor&s=wiki&a=[a]&dsd=ccid+{$g_cat_array['ddbdatalinker_id']}++[oid]");
		}
...
"uuid" => $direct_settings['uuid']
				...
				if ($g_doc_array['ddbdatalinker_symbol']) { $g_symbol_marked_object = new direct_datacenter (); }
				else { $g_symbol_marked_object = NULL; }

				if ($g_symbol_marked_object) { $g_symbol_marked_array = $g_symbol_marked_object->get_aid ("ddbdatacenter_plocation",$g_doc_array['ddbdatalinker_symbol']); }
				else { $g_symbol_marked_array = NULL; }
...
------------------------------------------------------------------------- */
		...
		if (isset ($direct_cachedata['i_csymbol'])) { $direct_classes['formbuilder']->entry_add_embed ("csymbol",(direct_local_get ("contentor_symbol")),false,"m=dataport&s=swgap;datacenter;selector_icons&dsd=","s"); }
...
------------------------------------------------------------------------- */
			...
			if ((isset ($direct_cachedata['i_csticky']))&&($direct_cachedata['i_csticky'])) { $direct_cachedata['i_csticky'] = 1; }
			else { $direct_cachedata['i_csticky'] = 0; }

			if ((isset ($direct_cachedata['i_clocked']))&&($direct_cachedata['i_clocked'])) { $direct_cachedata['i_clocked'] = 1; }
			else { $direct_cachedata['i_clocked'] = 0; }

			if ((isset ($direct_cachedata['i_cpublic']))&&($direct_cachedata['i_cpublic'])) { $direct_cachedata['i_cpublic'] = 1; }
			else { $direct_cachedata['i_cpublic'] = 0; }

			if (!isset ($direct_cachedata['i_cgright']))
			{
				if ((!$g_datasub_check)&&($g_cat_array['ddbcontentor_cats_owner_group']))
				{
					if ($g_cat_object->is_moderated ()) { $direct_cachedata['i_cgright'] = "r"; }
					else { $direct_cachedata['i_cgright'] = "w"; }
				}
			...
			if (!isset ($direct_cachedata['i_cpright']))
			{
				if (($g_doc_object->is_published ())&&(($g_datasub_check)||($g_cat_array['ddbcontentor_cats_public']))) { $direct_cachedata['i_cpright'] = "r"; }
				else { $direct_cachedata['i_cpright'] = "-"; }
			}

			if ((!$g_datasub_check)&&($g_cat_object->is_writable_as_submission ())) { $g_continue_check = true; }
			else { $g_continue_check = direct_credits_payment_check (false,$direct_settings['contentor_doc_edit_credits_onetime']); }

			if ($g_continue_check)
			{
				if (trim ($direct_cachedata['i_cteaser'])) { $direct_cachedata['i_cteaser'] = $direct_classes['formtags']->encode ($direct_cachedata['i_cteaser']); }
				else { $direct_cachedata['i_cteaser'] = ""; }
					...
					if ((!$g_datasub_check)&&($g_cat_object->is_diversity_dms ())&&(!$direct_cachedata['i_ceditcurrent']))
					{
						if ((!$g_cat_object->is_writable_as_submission ())&&($direct_cachedata['i_cpublic']))
						{
							...
							if ($g_doc_array['ddbdatalinker_type'] == 3)
							{
								...
								if ($g_doc_active_object) { $g_doc_active_array = $g_doc_active_object->get ($g_did); }
								else { $g_doc_active_array = $g_doc_array; }
					...
					if ($g_continue_check)
					{
						if (($direct_settings['contentor_datacenter_symbols'])&&(isset ($direct_settings['contentor_datacenter_symbols_did'])))
						{
							...
							if ((is_array ($g_task_array['datacenter_objects_marked']))&&(!empty ($g_task_array['datacenter_objects_marked'])))
							{
								...
								if ($g_symbol_marked_id) { $g_symbol_marked_object = new direct_datacenter (); }
								else { $g_symbol_marked_object = NULL; }
						...
						if (isset ($direct_cachedata['i_cviews_count']))
						{
							if ($direct_cachedata['i_cviews_count']) { $g_doc_array['ddbdatalinker_views_count'] = 1; }
							else { $g_doc_array['ddbdatalinker_views_count'] = 0; }
						}

						if (isset ($direct_cachedata['i_cfrontpage']))
						{
							if ($direct_cachedata['i_cfrontpage']) { $g_doc_array['ddbcontentor_docs_id_front'] = $g_cat_array['ddbcontentor_cats_id_front']; }
							else { $g_doc_array['ddbcontentor_docs_id_front'] = ""; }
						}
						...
						if ((!$g_datasub_check)&&($g_cat_object->is_writable_as_submission ())) { $g_doc_array['ddbcontentor_docs_public'] = 0; }
						else { $g_doc_array['ddbcontentor_docs_public'] = $direct_cachedata['i_cpublic']; }

						$g_doc_array['ddbdata_data'] = $direct_cachedata['i_ctext'];

						if ($g_insert_check) { $g_continue_check = $g_doc_object->set_insert ($g_doc_array); }
						else { $g_continue_check = $g_doc_object->set_update ($g_doc_array); }
...
case "new-save":
{
	...
	if ($g_cat_object)
	{
		...
		if ((!$g_cat_object->is_of_type ("87ecbe0ba0a0b3c7e60030043614e655",1))&&($direct_settings['user']['type'] != "gt"))
		{
			...
			$g_datasub_check = $g_cat_object->is_sub_allowed ();
		}

		if (($g_datasub_check)&&(isset ($direct_settings["contentor_".$g_type]))&&(in_array ($g_type,$direct_settings['contentor_subs_types_supported']))) { $g_continue_check = true; }
		elseif (($g_cat_array)&&(isset ($direct_settings["contentor_".$g_cat_array['ddbcontentor_cats_doctype']])))
		{
	...
	{
		if ($g_mode_save) { direct_output_related_manager ("contentor_doc_new_form_save","pre_module_service_action"); }
...
------------------------------------------------------------------------- */
			...
			if ($g_title) { $direct_cachedata['i_ctitle'] = base64_decode ($g_title); }
			else { $direct_cachedata['i_ctitle'] = ""; }
...
"uuid" => $direct_settings['uuid']
			...
			if ($g_rights_check)
			{
				if ($g_cat_object->is_views_counting ()) { $direct_cachedata['i_cviews_count'] = "<evars><yes><value value='1' /><selected value='1' /><text><![CDATA[".(direct_local_get ("core_yes"))."]]></text></yes><no><value value='0' /><text><![CDATA[".(direct_local_get ("core_no"))."]]></text></no></evars>"; }
				else { $direct_cachedata['i_cviews_count'] = "<evars><yes><value value='1' /><text><![CDATA[".(direct_local_get ("core_yes"))."]]></text></yes><no><value value='0' /><selected value='1' /><text><![CDATA[".(direct_local_get ("core_no"))."]]></text></no></evars>"; }

				if ($g_cat_array['ddbdatalinker_datasubs_new']) { $direct_cachedata['i_csubs_allowed'] = "<evars><yes><value value='1' /><selected value='1' /><text><![CDATA[".(direct_local_get ("core_yes"))."]]></text></yes><no><value value='0' /><text><![CDATA[".(direct_local_get ("core_no"))."]]></text></no></evars>"; }
				else { $direct_cachedata['i_csubs_allowed'] = "<evars><yes><value value='1' /><text><![CDATA[".(direct_local_get ("core_yes"))."]]></text></yes><no><value value='0' /><selected value='1' /><text><![CDATA[".(direct_local_get ("core_no"))."]]></text></no></evars>"; }
...
------------------------------------------------------------------------- */
			...
			if ((isset ($direct_cachedata['i_csubs_allowed']))&&($direct_cachedata['i_csubs_allowed'])) { $direct_cachedata['i_csubs_allowed'] = 1; }
			else { $direct_cachedata['i_csubs_allowed'] = 0; }
			...
			if (!isset ($direct_cachedata['i_cpright']))
			{
				if (($g_datasub_check)||($g_cat_array['ddbcontentor_cats_public'])) { $direct_cachedata['i_cpright'] = "r"; }
				else { $direct_cachedata['i_cpright'] = "-"; }
			}
...
"ddbdata_mode_all" => $direct_cachedata['i_cpright']
					...
					if ((isset ($direct_cachedata['i_cfrontpage']))&&($direct_cachedata['i_cfrontpage'])) { $g_insert_array['ddbcontentor_docs_id_front'] = $g_cat_array['ddbcontentor_cats_id_front']; }
					else { $g_insert_array['ddbcontentor_docs_id_front'] = ""; }
					...
					if ($g_doc_object) { $g_continue_check = $g_doc_object->set_insert ($g_insert_array); }
...
case "state":
{
	...
	if ($g_doc_array)
	{
		if (isset ($direct_settings["contentor_".$g_doc_array['ddbcontentor_docs_doctype']]))
		{
			if ($g_doc_array['ddbdatalinker_id_main'])
			{
				...
				if (($g_cat_array)&&($g_cat_object->is_moderator ())) { $g_rights_check = true; }
			}
			else
			{
				$g_datasub_check = true;
	...
	elseif ($g_rights_check)
	{
		if ($g_datasub_check) { direct_output_related_manager ("contentor_doc_state_{$g_did}_form","pre_module_service_action"); }
		else { direct_output_related_manager ("contentor_doc_state_{$g_cat_array['ddbdatalinker_id']}_{$g_did}_form","pre_module_service_action"); }
		...
		case "unlock":
		{
			...
			$g_continue_check = $g_doc_object->define_lock (false,true);
		...
		case "unstick":
		{
			...
			$g_continue_check = $g_doc_object->define_stick (false,true);
		...
		if ($g_datasub_check) { direct_output_related_manager ("contentor_doc_state_{$g_did}_form","post_module_service_action"); }
		else { direct_output_related_manager ("contentor_doc_state_{$g_cat_array['ddbdatalinker_id']}_{$g_did}_form","post_module_service_action"); }
