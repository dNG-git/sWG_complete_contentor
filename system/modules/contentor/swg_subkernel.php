...
if (!defined ("CLASS_direct_subkernel_contentor"))
{
//c// direct_subkernel_contentor
/**
* Subkernel for: contentor
*
* @author     direct Netware Group
...
*             GNU General Public License 2
*/
class direct_subkernel_contentor extends direct_virtual_class
{
/* -------------------------------------------------------------------------
Extend the class
------------------------------------------------------------------------- */
...
*/
	...
	{
		if (USE_debug_reporting) { direct_debug (5,"sWG/#echo(__FILEPATH__)# -kernel_class->__construct (direct_subkernel_contentor)- (#echo(__LINE__)#)"); }
...
------------------------------------------------------------------------- */
	...
	//f// direct_subkernel_contentor->subkernel_init ($f_threshold_id = "")
/**
	...
	public function subkernel_init ($f_threshold_id = "")
	{
		...
		if (empty ($f_return))
		{
			if ($direct_classes['db']->v_connect ())
			{
				...
				if ($direct_classes['basic_functions']->settings_get ($direct_settings['path_data']."/settings/swg_contentor.php"))
				{
					if (!$direct_settings['contentor']) { $f_return = array ("core_service_inactive","sWG/#echo(__FILEPATH__)# -kernel_class->subkernel_init ()- (#echo(__LINE__)#)"); }
				}
				else { $f_return = array ("core_required_object_not_found","FATAL ERROR:<br />&quot;$direct_settings[path_data]/settings/swg_contentor.php&quot; was not found<br /><br />sWG/#echo(__FILEPATH__)# -kernel_class->subkernel_init ()- (#echo(__LINE__)#)"); }
			}
...
$direct_classes['@names']['subkernel_contentor'] = "direct_subkernel_contentor";
define ("CLASS_direct_subkernel_contentor",true);

//j// Script specific commands

direct_class_init ("subkernel_contentor");
$direct_classes['kernel']->v_call_set ("v_subkernel_init",$direct_classes['subkernel_contentor'],"subkernel_init");
}
